...
*/
%%% loop_new to figure out what is going on. 
loop_new(Soc):-
	accept(Soc,_,S),
	catch(process(S),Tag,true),
	write(['loop tag:',Tag]),nl,
	close(S),
	!,loop_new(Soc).
...
%%% debugging Catch any Missing headers
parse_Params([H|T], TT):-
	write(['Failed on Header',H]),nl,
	!,
	parse_Params(T, TT).
